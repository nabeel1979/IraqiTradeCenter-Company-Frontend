...
const getSettingPaperSize = (paperType: string) => {
  ...
  for (let i = 0; i < PAPPER_PAGE_SIZE.length; i++) {
    const size = PAPPER_PAGE_SIZE[i]
    if (size.label === paperType || paperType.toUpperCase().includes(size.label.toUpperCase())) {
      width = size.Width
      height = size.Height
  ...
  // PWG/eSCL requires the region fields to be wrapped in <pwg:ScanRegion>
  // inside <pwg:ScanRegions>. Without this wrapper the scanner ignores the
  // requested size and falls back to its default (smaller) area, which cuts
  // off the bottom of the page.
  return {
    [SCAN_SETTING_KEY_MAP.ScanRegion]: {
      [SCAN_SETTING_KEY_MAP.Height]: height,
      [SCAN_SETTING_KEY_MAP.Width]: width,
      [SCAN_SETTING_KEY_MAP.XOffset]: xOffset,
      [SCAN_SETTING_KEY_MAP.YOffset]: yOffset,
