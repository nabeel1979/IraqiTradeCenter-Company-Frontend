...
import {
  ...
  FileText, Eye, Trash2, ListChecks,
} from 'lucide-react';
...
import { costReconciliationApi, type CostReconRow, type CostReconWarehouse, type CostSettlementEntrySummary } from '@/lib/api/costReconciliation';
...
export function MaterialCostProcessingPage() {
  ...
  // ── القيد المولّد آخر مرة (لفتحه أو حذفه من داخل المعالجة) ──
  const [generatedEntryId, setGeneratedEntryId] = useState<number | null>(null);
  const [deleteOpen, setDeleteOpen] = useState(false);
  // ── قائمة قيود التسوية (عرض/فتح/حذف) ──
  const [entriesOpen, setEntriesOpen] = useState(false);
  const [pendingDeleteEntry, setPendingDeleteEntry] = useState<CostSettlementEntrySummary | null>(null);
  ...
  const openRowMenu = useCallback((e: React.MouseEvent<HTMLButtonElement>, itemId: number, itemName: string, itemCode: string) => {
    ...
    const margin = 8;
    // تُفتح نحو يمين الصفحة (الحافة اليسرى للقائمة عند يمين الزر) مع ارتداد لليسار عند ضيق المساحة
    let x = rect.right + 4;
    if (x + menuWidth > window.innerWidth - margin) {
      x = rect.left - menuWidth - 4;
      if (x < margin) x = Math.max(margin, window.innerWidth - menuWidth - margin);
    }
    // تُفتح نحو الأعلى (أسفل القائمة فوق الزر) مع ارتداد للأسفل عند ضيق المساحة
    let y = rect.top - menuHeight - 4;
    if (y < margin) y = rect.bottom + 4;
    setRowMenu({ itemId, itemName, itemCode, x, y });
  }, []);
  ...
  // ── إعادة تقييم الكلفة: المواد التي ثُبّتت لها كلفة جديدة تختلف عن الحالية ──
  const revaluationLines = useMemo(
    () => rows
      .map(r => ({ r, nc: newCostFor(r) }))
      .filter(({ r, nc }) => {
        const raw = editedCosts[rowKey(r)];
        return raw != null && raw.trim() !== '' && Number.isFinite(nc) && Math.abs(nc - r.unitCost) >= 0.0005;
      })
      .map(({ r, nc }) => ({ itemId: r.itemId, warehouseId: r.warehouseId, newUnitCost: nc })),
    [rows, editedCosts, newCostFor],
  );
  ...
  const deleteMut = useMutation({
    mutationFn: (entryId: number) => costReconciliationApi.deleteSettlement(entryId),
    onSuccess: () => {
      toast.success(tt('تم حذف قيد التسوية واسترجاع الكلفة', 'Settlement entry deleted and cost restored'));
      setDeleteOpen(false);
      setGeneratedEntryId(null);
      void qc.invalidateQueries({ queryKey: ['cost-reconciliation'] });
      void reconQuery.refetch();
    },
    onError: (e: unknown) => toast.error(extractApiError(e) ?? tt('فشل حذف القيد', 'Failed to delete entry')),
  });

  const openGeneratedEntry = () => {
    if (generatedEntryId != null) navigate(`/accounting/journal/${generatedEntryId}/view`);
  };

  // ── قائمة قيود التسوية ──
  const settlementsQuery = useQuery({
    queryKey: ['cost-settlements'],
    queryFn: () => costReconciliationApi.listSettlements(),
    enabled: entriesOpen,
  });
  ...
  const listDeleteMut = useMutation({
    ...
    onSuccess: (_d, entryId) => {
      ...
      if (generatedEntryId === entryId) setGeneratedEntryId(null);
    ...
    <div className="space-y-4">
      ...
      {reconQuery.isLoading ? (
        ...
        <>
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-3 p-3.5">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  title={tt('قيود التسوية', 'Settlement entries')}
                >
                  <ListChecks className="h-4 w-4" /> {tt('قيود التسوية', 'Settlement entries')}
                </Button>
                <Button
                  size="sm"
                  className="h-9 gap-1.5"
          ...
          {/* ── القيد المولّد: فتحه أو حذفه ── */}
          {generatedEntryId != null && (
            <Card className="border-emerald-400/50 bg-emerald-50/40 dark:bg-emerald-900/10">
              <CardContent className="flex flex-wrap items-center justify-between gap-3 p-3.5">
                <div className="flex items-center gap-2 text-sm">
                  <FileText className="h-4 w-4 text-emerald-600" />
                  <span className="font-medium">{tt('قيد التسوية المولّد', 'Generated settlement entry')}</span>
                  <span className="num-display rounded bg-emerald-600/10 px-1.5 py-0.5 text-xs font-bold text-emerald-700 dark:text-emerald-300">#{generatedEntryId}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" className="h-8 gap-1.5" onClick={openGeneratedEntry}>
                    <Eye className="h-3.5 w-3.5" /> {tt('فتح القيد', 'Open entry')}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-8 gap-1.5 border-rose-300 text-rose-600 hover:bg-rose-50 dark:border-rose-800 dark:hover:bg-rose-900/20"
                    onClick={() => setDeleteOpen(true)}
                    disabled={deleteMut.isPending}
                  >
                    <Trash2 className="h-3.5 w-3.5" /> {tt('حذف القيد', 'Delete entry')}
                  </Button>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative flex w-full max-w-md flex-col rounded-lg border border-border bg-card shadow-xl" dir={locale === 'en' ? 'ltr' : 'rtl'}>
            ...
            <div className="space-y-3 px-4 py-4 text-sm">
              ...
              {revaluationLines.length > 0 && (
                <p className="flex items-start gap-1.5 rounded-md border border-violet-300 bg-violet-50 px-3 py-2 text-[11px] text-violet-800 dark:border-violet-800 dark:bg-violet-900/20 dark:text-violet-300">
                  <TrendingUp className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                  {tt(`سترتفع/تنخفض كلفة (${revaluationLines.length}) مادة في المستودع حسب الكلفة الجديدة المثبّتة.`,
                    `Cost of (${revaluationLines.length}) item(s) will be revalued to the new entered cost.`)}
                </p>
              )}
            </div>
      ...
      {/* ── نافذة تأكيد حذف القيد المولّد ── */}
      {deleteOpen && generatedEntryId != null && createPortal(
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => !deleteMut.isPending && setDeleteOpen(false)} />
          <div className="relative flex w-full max-w-md flex-col rounded-lg border border-border bg-card shadow-xl" dir={locale === 'en' ? 'ltr' : 'rtl'}>
            <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
              <h3 className="flex items-center gap-2 text-sm font-semibold text-rose-600">
                <Trash2 className="h-4 w-4" />
                {tt('حذف قيد التسوية', 'Delete settlement entry')}
              </h3>
              <button type="button" onClick={() => setDeleteOpen(false)} disabled={deleteMut.isPending} className="rounded-md p-1 text-muted-foreground hover:bg-accent disabled:opacity-40">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-3 px-4 py-4 text-sm">
              <div className="flex items-start gap-2 rounded-md border border-rose-300 bg-rose-50 px-3 py-2 text-xs text-rose-800 dark:border-rose-800 dark:bg-rose-900/20 dark:text-rose-300">
                <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                <span>
                  {tt('سيُحذف القيد رقم ', 'Entry #')}<b className="num-display">{generatedEntryId}</b>
                  {tt(' وسيُسترجع متوسط كلفة المواد التي أُعيد تقييمها بهذا القيد.', ' will be deleted and the revalued item costs will be restored.')}
                </span>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 border-t border-border px-4 py-3">
              <Button variant="outline" size="sm" onClick={() => setDeleteOpen(false)} disabled={deleteMut.isPending}>{tt('إلغاء', 'Cancel')}</Button>
              <Button variant="destructive" size="sm" className="gap-1.5" onClick={() => deleteMut.mutate(generatedEntryId)} disabled={deleteMut.isPending}>
                {deleteMut.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> {tt('جارٍ الحذف…', 'Deleting…')}</> : <><Trash2 className="h-4 w-4" /> {tt('تأكيد الحذف', 'Confirm delete')}</>}
              </Button>
            </div>
          </div>
        </div>,
      ...
      {/* ── نافذة قائمة قيود التسوية: عرض/فتح/حذف ── */}
      {entriesOpen && createPortal(
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setEntriesOpen(false)} />
          <div className="relative flex max-h-[85vh] w-full max-w-2xl flex-col rounded-lg border border-border bg-card shadow-xl" dir={locale === 'en' ? 'ltr' : 'rtl'}>
            <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
              <h3 className="flex items-center gap-2 text-sm font-semibold">
                <ListChecks className="h-4 w-4 text-primary" />
                {tt('قيود التسوية', 'Settlement entries')}
              </h3>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => settlementsQuery.refetch()} disabled={settlementsQuery.isFetching} className="rounded-md p-1 text-muted-foreground hover:bg-accent disabled:opacity-40" title={tt('تحديث', 'Refresh')}>
                  <RefreshCw className={cn('h-4 w-4', settlementsQuery.isFetching && 'animate-spin')} />
                </button>
                <button type="button" onClick={() => setEntriesOpen(false)} className="rounded-md p-1 text-muted-foreground hover:bg-accent">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
            <div className="overflow-auto px-4 py-3">
              ...
              ) : (settlementsQuery.data?.length ?? 0) === 0 ? (
                <div className="py-10"><EmptyState icon={ClipboardList} title={tt('لا توجد قيود تسوية', 'No settlement entries')} /></div>
              ) : (
                <table className="w-full text-xs">
                  <thead className="bg-muted/60 text-muted-foreground">
                    <tr>
                      <th className="px-2 py-2 text-right font-medium">{tt('رقم القيد', 'Entry #')}</th>
                      <th className="px-2 py-2 text-right font-medium">{tt('التاريخ', 'Date')}</th>
                      <th className="px-2 py-2 text-right font-medium">{tt('البيان', 'Description')}</th>
                      <th className="px-2 py-2 text-center font-medium">{tt('المبلغ', 'Amount')}</th>
                      <th className="px-2 py-2 text-center font-medium">{tt('الإجراءات', 'Actions')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {settlementsQuery.data!.map(s => {
                      ...
                      return (
                        <tr key={s.entryId} className="border-t border-border/40 hover:bg-accent/30">
                          <td className="px-2 py-1.5 num-display font-medium text-emerald-600">{s.entryNumber}</td>
                          <td className="px-2 py-1.5">{s.entryDate.slice(0, 10)}</td>
                          <td className="px-2 py-1.5 max-w-[220px] truncate" title={s.description}>{s.description}</td>
                          <td className="px-2 py-1.5 text-center num-display font-semibold">{formatAmountFixed2(s.amount)} {s.currency}</td>
                          <td className="px-2 py-1.5">
                            <div className="flex items-center justify-center gap-1.5">
                              <button
                                type="button"
                                className="inline-flex items-center gap-1 rounded border px-2 py-1 text-[11px] hover:bg-accent"
                                onClick={() => { setEntriesOpen(false); navigate(`/accounting/journal/${s.entryId}/view`); }}
                              >
                                <Eye className="h-3.5 w-3.5 text-primary" /> {tt('عرض القيد', 'View entry')}
                              </button>
                              <button
                                type="button"
                                disabled={reversed || listDeleteMut.isPending}
                                className="inline-flex items-center gap-1 rounded border border-rose-300 px-2 py-1 text-[11px] text-rose-600 hover:bg-rose-50 disabled:opacity-40 dark:border-rose-800 dark:hover:bg-rose-900/20"
                                onClick={() => setPendingDeleteEntry(s)}
                                title={reversed ? tt('قيد معكوس — لا يُحذف', 'Reversed — cannot delete') : tt('حذف', 'Delete')}
                              >
                                <Trash2 className="h-3.5 w-3.5" /> {tt('حذف', 'Delete')}
                              </button>
                            </div>
                          </td>
                        </tr>
      ...
      {/* ── تأكيد حذف قيد من القائمة ── */}
      {pendingDeleteEntry && createPortal(
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => !listDeleteMut.isPending && setPendingDeleteEntry(null)} />
          <div className="relative flex w-full max-w-md flex-col rounded-lg border border-border bg-card shadow-xl" dir={locale === 'en' ? 'ltr' : 'rtl'}>
            <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
              <h3 className="flex items-center gap-2 text-sm font-semibold text-rose-600">
                <Trash2 className="h-4 w-4" />
              ...
              <button type="button" onClick={() => setPendingDeleteEntry(null)} disabled={listDeleteMut.isPending} className="rounded-md p-1 text-muted-foreground hover:bg-accent disabled:opacity-40">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-3 px-4 py-4 text-sm">
              <div className="flex items-start gap-2 rounded-md border border-rose-300 bg-rose-50 px-3 py-2 text-xs text-rose-800 dark:border-rose-800 dark:bg-rose-900/20 dark:text-rose-300">
                <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                <span>
                  {tt('سيُحذف القيد ', 'Entry ')}<b className="num-display">{pendingDeleteEntry.entryNumber}</b>
                  {tt(' وسيُسترجع متوسط كلفة المواد التي أُعيد تقييمها بهذا القيد.', ' will be deleted and the revalued item costs will be restored.')}
                </span>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 border-t border-border px-4 py-3">
              <Button variant="outline" size="sm" onClick={() => setPendingDeleteEntry(null)} disabled={listDeleteMut.isPending}>{tt('إلغاء', 'Cancel')}</Button>
              <Button variant="destructive" size="sm" className="gap-1.5" onClick={() => listDeleteMut.mutate(pendingDeleteEntry.entryId)} disabled={listDeleteMut.isPending}>
                {listDeleteMut.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> {tt('جارٍ الحذف…', 'Deleting…')}</> : <><Trash2 className="h-4 w-4" /> {tt('تأكيد الحذف', 'Confirm delete')}</>}
              </Button>
            </div>
          </div>
        </div>,
