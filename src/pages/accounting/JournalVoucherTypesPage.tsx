...
import { suggestNextVoucherTypeCode } from '@/lib/accounting/voucherTypeCode';
...
function VoucherTypeDialog({
  ...
  const suggestedCode = useMemo(() => suggestNextVoucherTypeCode(existingCodes), [existingCodes]);
  const [code, setCode] = useState(existing?.code ?? suggestedCode);
  ...
  const generateCodeQuery = useQuery({
    queryKey: ['voucher-types', 'generate-code', existingCodes.join(',')],
    queryFn: () => journalVoucherTypesApi.generateCode(),
    enabled: isNew,
    staleTime: 0,
    retry: false,
  });
  ...
  useEffect(() => {
    if (isNew) setCode(suggestedCode);
  }, [isNew, suggestedCode]);

  useEffect(() => {
    if (isNew && generateCodeQuery.data) setCode(generateCodeQuery.data);
  }, [isNew, generateCodeQuery.data]);
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-lg border border-border bg-card shadow-2xl" dir="rtl">
        <div className="flex shrink-0 items-center justify-between border-b border-border bg-secondary/30 px-4 py-2.5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              ...
              <Input
                ...
                readOnly
                disabled
                className={cn('h-9 cursor-not-allowed bg-secondary/50 text-sm font-mono', codeError && 'border-destructive')}
              />
              {codeError && <p className="mt-0.5 text-[10px] text-destructive">{codeError}</p>}
              {isNew && !codeError && (
                <p className="mt-0.5 text-[10px] text-muted-foreground">{t('voucherTypes.dialog.codeAutoGenerated')}</p>
              )}
        ...
        <div className="flex shrink-0 items-center justify-end gap-2 border-t border-border bg-secondary/20 px-4 py-2.5">
